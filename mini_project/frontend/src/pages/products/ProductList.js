...
const ProductList = () => {
  ...
  const fetchProducts = async (page = 1, search = '', sort = `${sortDirection === 'asc' ? '' : '-'}${sortField}`) => {
    try {
      ...
      if (response.data.data && Array.isArray(response.data.data)) {
        checkLowStock(response.data.data);
      }
    } catch (err) {
  ...
  const handleStockUpdate = (productId) => {
    ...
    // Refresh products after a short delay to allow for backend processing
    setTimeout(() => {
  ...
  const testLowStockRemoval = (product) => {
    console.log('Testing low stock removal for product:', product);
    const updatedProduct = {
      _id: product._id,
      name: product.name,
      modelNumber: product.modelNumber,
      stockQuantity: product.stockQuantity + 10, // Increase stock by 10
      lowStockThreshold: product.lowStockThreshold
    };
    console.log('Simulated updated product:', updatedProduct);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => {
        const isUpdating = updatingProducts.has(row._id);
        const isLowStock = row.stockQuantity <= row.lowStockThreshold;
      ...
      cell: (row) => (
        <div className="flex space-x-2">
          <Link to={`/products/edit/${row._id}`}>
            <button className="text-indigo-600 hover:text-indigo-900" title="Edit Product">
              <PencilIcon className="h-5 w-5" />
            </button>
          </Link>
          {row.stockQuantity <= row.lowStockThreshold && (
            <button 
              onClick={() => testLowStockRemoval(row)} 
              className="text-green-600 hover:text-green-900" 
              title="Test Low Stock Removal"
            >
              Test
            </button>
          )}
